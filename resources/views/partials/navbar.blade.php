<nav class="navbar navbar-expand-lg navbar-dark bg-info">
    <div class="container">
      <a class="navbar-brand" href="/">Inventaris TU JTIf</a>
      <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
        <span class="navbar-toggler-icon"></span>
      </button>
      <div class="collapse navbar-collapse" id="navbarNav">
        <ul class="navbar-nav">
          <li class="nav-item">
            <a class="nav-link {{ ($title === "Dashboard") ? 'active' : '' }}" href="/dashboard">Dashboard</a>
          </li>
          <li class="nav-item dropdown">
            <a class="nav-link dropdown-toggle" href="#" id="navbarDropdownMenuLink" role="button" data-bs-toggle="dropdown">
              Master Data
            </a>
            <ul class="dropdown-menu">
              <li><a class="dropdown-item" href="/inventaris">Data Inventaris</a></li>
              <li><a class="dropdown-item" href="/lokasi">Lokasi</a></li>
              <li><a class="dropdown-item" href="/categories">Kategori</a></li>
            </ul>
          </li>
          <li class="nav-item">
            <a class="nav-link {{ ($title === "Peminjaman") ? 'active' : '' }}" href="/peminjaman">Peminjaman</a>
          </li>
        </ul>
      </div>
    </div>
  </nav>